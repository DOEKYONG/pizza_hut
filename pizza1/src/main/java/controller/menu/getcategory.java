...
public class getcategory extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 반환타입 변수 호출
				...
				// DB에서 카테고리 리스트 호출 
				ArrayList<Category> arrayList = MenuDao.getmemberDao().getcategorylist();
				...
				if( type !=null && type.equals("option") ) { // option 반환
					...
					for( Category temp : arrayList ) {
						html +=
							"<option value=\""+temp.getCnum()+"\">"+temp.getCname()+"</option>";
					}
					...
					for( Category temp  : arrayList  ) {
						html += 
						"<input type=\"radio\" name=\"cnum\" value=\""+
						temp.getCnum()+"\">"+temp.getCname();
